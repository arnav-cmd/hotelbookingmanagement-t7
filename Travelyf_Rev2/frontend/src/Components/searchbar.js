...
export default function Search2() {
  ...
  if(checkindate && checkoutdate){
    daydifference=differenceInDays( new Date(checkoutdate),new Date(checkindate));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      dispatch(dates({checkin:formatDate(checkindate),checkout:formatDate(checkoutdate),rooms:rooms,daydiff:daydifference}))
    } catch (error) {
