...
const AdminForm = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if(!adminImageurl||!admincity||!adminhotel||!adminarea||!adminrating||!admincost||!adminrooms){
        setadminError("Fill all the fields");
    }
    else{
        setadminError("");
    ...
    .then((response)=>{
        ...
        if(response.data.HotelName==="exist"){
            setadminError("Hotel already exists");
        }
        else{
            setadminError("Details enteres successfully");
        }
    })
    }
